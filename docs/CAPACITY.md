...
| 항목 | 요금 | 계산 | 회당 비용 |
|---|---|---|---|
| Video HD (720p) | $3.99 / 1000 분 = $0.00399/분 | 응시자 100 × 120분 = 12,000분 · 감독관 2 × 120분 = 240분 | **12,240분 × $0.00399 = $48.8** |
| **SD (480p 저해상도)** — 권장 | $0.99 / 1000 분 | 감독관 그리드는 SD 썸네일이면 충분 | **12,240분 × $0.00099 = $12.1** |
| Free tier | 첫 **10,000분/월 무료** | 100명 규모면 첫 회 상당 부분이 무료 | 첫 회 실질 $2~4 |
| Voice-only fallback | $0.99 / 1000분 | 스트림 문제 시 오디오만 | 대체 |

**계산 (SD 그리드 기준)**: **회당 약 $12 = 약 1.7만원**  
Free tier 활용 시 첫 회 **약 3천원~5천원**.

**Agora 선택 이유**:
- Daily.co Prime 대비 **약 1/6 비용**
- Web SDK(`agora-rtc-sdk-ng`) 성숙 · React 예제 많음
- 한국 리전 지원 (Seoul edge)
- Free 10,000분/월 → 초기 개발·리허설에 유리

**주의**:
- Agora 채팅은 Signaling SDK 별도(RTM). 승우님 감독관↔응시자 채팅은 **Supabase Realtime으로 자체 구현** (Agora 비용 절감)
...
|---|---|---|
| Supabase Pro | — | 3.5만원 |
| **Agora (SD 그리드)** | **약 1.7만원** | — |
| **Cloudflare R2 (5s 청크)** | 약 4천원 | — |
| Resend Free | 0원 | 0원 |
| Vercel Pro | — | 3만원 |
| **합계** | **약 2.1만원 / 회** | **약 6.5만원 / 월** |

**월 1회 시험 기준**: **회당 총 약 8.6만원** · 응시자 1인당 **약 860원**  
**월 2회 시험 기준**: **회당 총 약 6.5만원** · 응시자 1인당 **약 650원**  
**월 4회 시험 기준**: **회당 총 약 3.7만원** · 응시자 1인당 **약 370원**
...
| # | 항목 | 위험 | 실행 방법 |
|---|---|---|---|
| 1 | **Agora 계정 생성 · 프로젝트 세팅** | Seoul 리전 App ID 발급 · Web SDK 통합 | agora.io 계정 → Console에서 프로젝트 생성 → App ID/Certificate 발급 |
...
- **회당 총 약 8.6만원 · 응시자 1인당 약 860원** (인건비 제외)
- Daily.co → **Agora로 화상회의 비용 1/6로 절감**
- 실기기 리허설 규모도 30~50명 이내로 현실적
- 첫 시험도 100명 그대로 배포 가능
- M1 진입 전 미결정 4개(R2 계정 · 이메일 · 얼굴 모델 · 보관기간)만 확정하면 됨
