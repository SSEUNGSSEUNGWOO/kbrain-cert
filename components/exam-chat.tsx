    ...
    <div className="fixed right-6 top-[76px] z-40">
      {!visibleOpen && (
        <button
          ...
          className="relative h-9 px-4 rounded-md bg-primary hover:bg-primary-hover text-white shadow-md flex items-center gap-2 text-xs font-bold transition"
        >
          <span>감독관 채팅</span>
          {unreadCount > 0 && (
            <span className="absolute -top-1 -right-1 w-5 h-5 rounded-full bg-danger text-white text-[10px] font-bold flex items-center justify-center border-2 border-white animate-pulse font-tabular">
      ...
      {visibleOpen && (
        <div className="absolute top-0 right-0 w-80 rounded-md bg-white border border-border shadow-xl overflow-hidden flex flex-col max-h-[400px]">
